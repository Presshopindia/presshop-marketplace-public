                  ...
                  <div className="notf_wrp" onClick={() => read(curr?._id)}>
                    <div className="notf_item active">
                      {curr?.sender_id?.role === "Hopper" ?
                        <img src={process.env.REACT_APP_AVATAR_IMAGE + curr?.sender_id?.avatar_id?.avatar} className="notf_img" alt="" />
                        : <img src={process.env.REACT_APP_ADMIN_IMAGE + curr?.sender_id?.profile_image} className="notf_img" alt="" />}
                      <div className="notf_cont_rt">
                        <p className="notf_usr d-flex align-items-center justify-content-between">
                          {curr?.sender_id?.role === "Hopper" ?
                            curr?.sender_id?.user_name :
                            curr?.sender_id?.name}
                          <span className="notf_time_txt">{moment(curr?.createdAt).format(`hh:mm A`)}</span>
                        </p>
