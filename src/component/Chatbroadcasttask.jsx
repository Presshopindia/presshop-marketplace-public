              ...
              <div className="taskcard_body ps-0 pe-0 pb-0">
                <div className="mapInput mapView position-relative">
                  <GoogleMap
                    ...
                    center={{ lat: taskDetails?.address_location?.coordinates[0] ? taskDetails?.address_location?.coordinates[0] : latitude, lng: taskDetails?.address_location?.coordinates[1] ? taskDetails?.address_location?.coordinates[1] : longitude }}
                    zoom={8}
                    mapContainerStyle={{ height: '400px', width: '100%' }}
                  >
                    {taskDetails?.address_location?.coordinates &&
                      <Marker
                        position={{ lat: taskDetails?.address_location?.coordinates[0], lng: taskDetails?.address_location?.coordinates[1] }}
                      />
                    }
                  </GoogleMap>
                ...
                <div className="taskInfo_card">
                  ...
                  <div className="taskSingle_info">
                    <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M9.58901 0.693055L14.6514 2.94108C16.11 3.5846 16.11 4.64855 14.6514 5.29206L9.58901 7.54009C9.01414 7.79749 8.07031 7.79749 7.49543 7.54009L2.43309 5.29206C0.974451 4.64855 0.974451 3.5846 2.43309 2.94108L7.49543 0.693055C8.07031 0.435648 9.01414 0.435648 9.58901 0.693055Z" stroke="#7D8D8B" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round" />
                      <path d="M1 7.62598C1 8.34672 1.54056 9.179 2.20123 9.47073L8.02722 12.062C8.47339 12.2593 8.97962 12.2593 9.41721 12.062L15.2432 9.47073C15.9039 9.179 16.4444 8.34672 16.4444 7.62598" stroke="#7D8D8B" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round" />
                      <path d="M1 11.916C1 12.714 1.47191 13.4347 2.20123 13.7608L8.02722 16.352C8.47339 16.5493 8.97962 16.5493 9.41721 16.352L15.2432 13.7608C15.9725 13.4347 16.4444 12.714 16.4444 11.916" stroke="#7D8D8B" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                  ...
                  <Row className='justify-content-between rw_gp_5'>
                    <Col md={6} className='pe-0'>
                      <div className="timeSlots_tiles">
                        <label>Date</label>
                        <span className='sm-tiles'><img className='tsk_dlt_icns' src={calendaric} alt='date' />{moment(taskDetails?.deadline_date).format("DD/MM/YYYY") !== "Invalid date" ? " " + moment(taskDetails?.deadline_date).format("DD/MM/YYYY") : "dd/mm/yyyy"}</span>
                      </div>
                    </Col>
                    <Col md={6} className='pe-0'>
                      <div className="timeSlots_tiles">
                        <label>Deadline</label>
                        <span className='sm-tiles'><img className='tsk_dlt_icns' src={timeic} alt="time" />{moment(taskDetails?.deadline_date).format("hh:mm A") !== "Invalid date" ? " " + moment(taskDetails?.deadline_date).format("hh:mm A") : " 00:00 AM"}</span>
                      </div>
                    </Col>
                  ...
                  <Row className='mt-2'>
                    ...
                    <Col md={6} className='d-flex justify-content-end'>
                      <div className="taskUploads_media" onClick={() => navigate(`/Uploaded-Content/${taskDetails?._id}`)}>
                        ...
                        <Link className='text-dark' to={`/Uploaded-Content/${taskDetails?._id}`} ><small className='font-bold view_tsk_link'>View all uploaded content </small><BsArrowRight className='text-pink float-end me-2' /></Link>
                      </div>
                    </Col>
                  </Row>
                </div>
