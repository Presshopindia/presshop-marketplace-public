  ...
  const TaskDetails = async (id) => {
    ...
    try {
      const resp = await Get(
        `mediaHouse/live/expired/tasks?status=live&id=${id || param.id}`
      );
              ...
              <div className="taskcard_body ps-0 pe-1 pb-0">
                ...
                <div className="mapInput mapView position-relative">
                  ...
                  <div className="hopper_content">
                    <span className="me-4">
                      <MdMyLocation />{" "}
                      {taskDetails.hasOwnProperty("accepted_by") ? taskDetails?.accepted_by?.length === 0 ||
                      taskDetails?.accepted_by?.length === 1
                        ? `${taskDetails?.accepted_by?.length} Hopper tasked`
                        : `${taskDetails?.accepted_by?.length} Hoppers tasked` : `${0} Hopper tasked`}{" "}
                    </span>
                    <span>
