...
const FavouritedContent = () => {
  ...
  const FavouriteContent = async () => {
    ...
    try {
      ...
      setFavContent({...favContent, data: resp?.data?.response?.response, categoryData: category?.data?.data});
      if (resp) {
        setLoading(false);
      }
    } catch (error) {
                ...
                <div className="sorting_wrap d-flex">
                  <div className="feedSorting me-4">
                    <div className="fltrs_prnt top_fltr">
                      <button className='sortTrigger' onClick={() => setFavContent({ ...favContent, filter: { ...favContent.filter, filter: favContent.filter.filter == "true" ? "false" : "true" } })} >Filter <AiFillCaretDown /></button>
                      {
                        favContent?.filter?.filter == "true" && <ContentUnderOfferFilter setContentUnderOffer={setFavContent} contentUnderOffer={favContent} favDev={false} />
                      }
                    </div>
                  </div>
                  <div className="feedSorting">
                    <div className="fltrs_prnt top_fltr">
                      ...
                      <button className='sortTrigger' onClick={() => setFavContent({ ...favContent, sort: { ...favContent.sort, sort: favContent.sort.sort == "true" ? "false" : "true" } })}>Sort <AiFillCaretDown /></button>
                      {
                        favContent?.sort?.sort == "true" && <UnderOfferSort setContentUnderOffer={setFavContent} contentUnderOffer={favContent} />
                      }
                    </div>
                  </div>
                </div>
